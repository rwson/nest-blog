...
const ArticleRubbishList: React.FC = () => {
  ...
  const queryArticles = React.useCallback(async(page?: number) => {
    ...
    const res = await Http.get<QueryTagArticleData>(article.list('rubbish'), {
  ...
  const recoveryArticle = React.useCallback((id: string) => {
    Modal.confirm({
      title: '恢复',
      content: '确定要恢复此篇文章吗?',
      cancelText: '取消',
      okText: '确定',
      onOk: async () => {
        const res = await Http.post(article.recovery(id));
      ...
      },
      {
        ...
        render: (text: string, row: ArticleDetailData) => (
          <>
            ...
            <Button size="small" type="primary" onClick={() => recoveryArticle(row.id)}>
              恢复
            </Button>
            <Divider type="vertical" />
